...
finds $x_{ij}$ so that 
\[
\rm{left}(x_{ij}):\{\bm{x}_k,y_k:~x_{kj} \leq x_{ij}\}
\text{~~and~~}\rm{right}(x_{ij}):\{\bm{x}_k,y_k:~x_{kj} > x_{ij}\}
\]
...
\[
n\sigma^2(x, \bs{\theta} ) = \sum_{k \in \mathrm{left}(x)} \theta_k (y_k - \mu_{\mathrm{left}(x)})^2 + \sum_{k \in \mathrm{right}(x)} \theta_k (y_k - \mu_{\mathrm{right}(x)})^2
\]
where $\theta_k$ are observation weights {\gr ($\bar y_\star$ are calculated w.r.t. $\bs{\theta}$)}.
