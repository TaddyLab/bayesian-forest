...
\vskip .25cm
After observing $\bm{Z} = \{\bm{z}_1 \dots \bm{z}_n\}$, the posterior has $\theta_l \sim \mr{Exp}(a+1)$ if $\bm{\zeta}_l = \bm{z}_i$ for some $i$, and $\mr{Exp}(a)$ otherwise {\gr (say every $\bm{z}_i$ is unique)}.

\vskip .5cm
The {\nv limiting non-informative prior} takes $a \to 0$.  

\vskip .15cm
This leads to $\mr{p}(\theta_l = 0) = 1$ if $\bs{\zeta}_l \notin \bm{Z}$.
...
{\bf Example: Ordinary Least Squares}
...
{\theme This is a random variable.}

\vskip .25cm  We could sample from the posterior via bootstrapping.

\vskip .25cm Or, we can derive posterior moments for a first-order approx
\[
\bs{\tilde \beta} = \bs{\hat \beta} + 
\nabla \bs{\beta}\big |_{\bs{\theta}=\bm{1}} (\bs{\theta} - \bm{1})
\]
where $\bs{\hat \beta} = [\bm{X}'\bm{X}]^{-1}\bm{X}'y$ is OLS at the posterior mean DGP.
...
{\bf{\theme Example 1:} User-Specific Behavior in Experiments}
...
For example, $\gamma_{new\_user}$ vs $\gamma_{purch\_lastyear}$ 

\begin{center}
\includegraphics[width=\textwidth]{../graphs/gammapost}
\end{center}
...
\vskip .5cm
\includegraphics[width=0.5\textwidth]{../graphs/highlights}
\includegraphics[width=0.5\textwidth]{../graphs/highlights_cap}
